...
router.route('/restaurantResults').post(async (req, res) => {
  ...
  if (!req.body.searchVenueTerm || req.body.searchVenueTerm.trim() === "") {
    return res.status(400).render('../views/error', {title: "Error: 400", error: "Error 400: No input text was given into the form."});
  }
  try {
    let ob = await getVenues(req.body.searchVenueTerm)
    let venues = ob._embedded.venues;
    return res.render('../views/venueSearchResults', {title: "Venues Found", searchVenueTerm: req.body.searchVenueTerm, venues: venues});
  } catch (e) {
    return res.status(404).render('../views/venueNotFound', {title: "Error 404", searchVenueTerm: req.body.searchVenueTerm});
  }
});
...
router.route('/restaurantDetails/:id').get(async (req, res) => {
  ...
  if (!req.params.id || req.params.id.trim() === "") {
    return res.status(404).render("../views/error", {title: "Error 404", error: "Error 404: A venue with the given id does not exist."});
  }
  try {
    ...
    let address2 = "N/A";
    if (Object.keys(venue).includes("boxOfficeInfo") && Object.keys(venue.boxOfficeInfo).includes("phoneNumberDetail")) {
    ...
    return res.render("../views/venueByID", {title: "Venue Details", name: name, src: src, website: website, address1: address1, address2: address2});
  } catch (e) {
    return res.status(404).render("../views/error", {title: "Error 404", error: "Error 404: A venue with the given id does not exist."});
  }
});
